...
export class ShellComponent implements OnInit, OnDestroy, OnChanges{
 ...
 and examples.');
    ...
    handler2(value): string {
        console.log('handler2');

        let arr = value.split(this.re);
        let data = arr[0];
        for (let i = 1; i < arr.length; i++) {
            data += arr[i].substring(1);
        }
        return data;
    }
    handler(value) {
    ...
    initializeTerminal() {
        ...
        this.ss.shellOutput.subscribe((value) => {
            ...
            const trimedValue = this.handler2(value);
            if (trimedValue == this.pre_command) {
                value = trimedValue;
            }
            console.log(this.pre_command.charCodeAt(0));
            
            if (this.pre_data.charCodeAt(0) === 27) {
                console.log('key27');
    ...
    (<any>this.term).attachCustomKeyEventHandler(function (e) {
        console.log(e);
