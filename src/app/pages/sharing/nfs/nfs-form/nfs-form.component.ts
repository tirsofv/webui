...
export class NFSFormComponent {
  ...
  preInit(EntityForm: any) {
    ...
    this.route.params.subscribe(params => {
      if(params['pk']) {
        ...
        this.rest.get(`/sharing/nfs/${this.pk}`, {}).subscribe(console.log);
      }
    });
  ...
  nfs_hosts_event(parent: any){
      ...
      if(parent.entityForm) {
        if(parent.entityForm.formGroup.controls['hosts'].value !=='') {
        const network_string = parent.entityForm.formGroup.controls['hosts'].value.split(/[\s,]+/);
        let error_msg = ""
